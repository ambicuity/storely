import type { StorelyMemoryAdapter } from "@ambicuity/storely-core";
...
describe("createStorely", () => {
	it("should create a Storely instance with BigMap adapter", () => {
		const storely = createStorely();
		expect(storely).toBeDefined();
		expect((storely.store as StorelyMemoryAdapter).store).toBeInstanceOf(BigMap);
	});

	it("should create a Storely instance with custom options", () => {
		const storely = createStorely({ storeSize: 8 });
		expect(storely).toBeDefined();
		expect((storely.store as StorelyMemoryAdapter).store).toBeInstanceOf(BigMap);
		expect(
			((storely.store as StorelyMemoryAdapter).store as BigMap<string, unknown>).storeSize,
		).toBe(8);
	});

	it("should work with set and get operations", async () => {
		const storely = createStorely<string, number>();
		await storely.set("testKey", 123);
		const value = await storely.get<number>("testKey");
	...
	it("should work with custom hash function", async () => {
		...
		await storely.set("test", "value");
		const value = await storely.get("test");
