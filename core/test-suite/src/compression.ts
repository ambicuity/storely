import Storely, { type StorelyCompressionAdapter } from "@ambicuity/ambicore";
 ...
 * with arrays, strings, numbers, and integration with a Storely instance.
 * @param test - The test registration function (e.g. vitest `it`)
 * @param compression - The compression adapter instance to test
 */
const compressionTestSuite = (test: TestFunction, compression: StorelyCompressionAdapter) => {
	...
	test("compress/decompress with main storely", async (t) => {
		const storely = new Storely({ compression });
