...
/**
 * Configuration options for StorelyBridgeAdapter.
 */
export type StorelyBridgeAdapterOptions = {
	/**
...
/**
 * Interface for a promise-based store that can be used with StorelyBridgeAdapter.
 ...
 */
export type StorelyBridgeStore = {
...
/**
 * A bridge storage adapter for Storely that wraps any promise-based store.
 *
 ...
 * const bridge = new StorelyBridgeAdapter(myAsyncStore, { namespace: 'cache' });
 *
 * // Using with Storely
 * const storely = new Storely({ store: new StorelyBridgeAdapter(myAsyncStore) });
 * ```
 */
export class StorelyBridgeAdapter extends Hookified implements StorelyStorageAdapter {
	private _store: StorelyBridgeStore;
	private _namespace?: string;
	private _keySeparator = ":";
	private readonly _capabilities: StorelyStorageCapability;

	/**
	 * Creates a new StorelyBridgeAdapter instance.
	 * @param store - The underlying promise-based store to bridge
	 * @param options - Configuration options for the adapter
	 */
	constructor(store: StorelyBridgeStore, options?: StorelyBridgeAdapterOptions) {
		...
		if (typeof store.on === "function") {
			store.on(StorelyEvents.ERROR, (error: any) => this.emit(StorelyEvents.ERROR, error));
		}
	}
	 ...
	 */
	public set store(store: StorelyBridgeStore) {
		this._store = store;
	}
	...
	public async get<T>(key: string): Promise<StorelyStorageGetResult<T>> {
		...
		return data as StorelyStorageGetResult<T>;
	}
	...
	public async getMany<T>(keys: string[]): Promise<Array<StorelyStorageGetResult<T | undefined>>> {
		if (this._capabilities.methods.getMany.exists) {
			...
			for (const [index, data] of results.entries()) {
				...
				values.push(data as StorelyStorageGetResult<T | undefined>);
			}
