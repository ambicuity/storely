...
import { detectStorelyStorage, type StorelyStorageCapability } from "../capabilities.js";
import { Storely } from "../storely.js";
import type { StorelyStorageAdapter, StorelyStorageGetResult } from "../types/adapters.js";
import { type StorelyEntry, StorelyEvents } from "../types/storely.js";
...
/**
 * Configuration options for StorelyMemoryAdapter.
 */
export type StorelyMemoryAdapterOptions = {
	/**
...
/**
 * Interface for a Map-like store that can be used with StorelyMemoryAdapter.
...
/**
 * An in-memory storage adapter for Storely that wraps any Map-like object.
 *
 * This class provides a unified interface for using various Map-like stores
 * with Storely, handling namespace prefixing, TTL-based expiration, and batch operations.
 *
 * @example
 * ```typescript
 * // Using with a standard Map
 * const store = new StorelyMemoryAdapter(new Map(), { namespace: 'cache' });
 *
 * // Using with a custom store
 * const customStore = new StorelyMemoryAdapter(myCustomMapLikeStore, {
 *   namespace: 'tenant-123',
 *   keySeparator: ':'
 * });
 * ```
 */
export class StorelyMemoryAdapter extends Hookified implements StorelyStorageAdapter {
	private _store: StorelyMapType;
	private _namespace?: string;
	private _keySeparator = ":";
	private readonly _capabilities: StorelyStorageCapability;

	/**
	 * Creates a new StorelyMemoryAdapter instance.
	 ...
	 */
	public get capabilities(): StorelyStorageCapability {
	 ...
	 */
	public set store(store: StorelyMapType) {
		this._store = store;
	}
	 ...
	 */
	public async get<T>(key: string): Promise<StorelyStorageGetResult<T>> {
		...
		return entry.value as StorelyStorageGetResult<T>;
	}
	 ...
	 */
	public async getMany<T>(keys: string[]): Promise<Array<StorelyStorageGetResult<T | undefined>>> {
		const values: Array<StorelyStorageGetResult<T | undefined>> = [];
		for (const key of keys) {
			const keyPrefix = this.getKeyPrefix(key, this._namespace);
			const entry = this._store.get(keyPrefix) as MemoryEntry | undefined;
			if (entry === undefined || entry === null) {
				values.push(undefined as StorelyStorageGetResult<T | undefined>);
				continue;
			}
			...
			values.push(entry.value as StorelyStorageGetResult<T | undefined>);
		}
...
/**
 * Creates a Storely instance with a memory adapter optimized for in-memory storage.
 *
 * This factory function configures Storely to bypass serialization/deserialization
 ...
 * @returns A configured Storely instance with optimized settings for in-memory storage
 *
 * @example
 * ```typescript
 * // Create a simple in-memory cache
 * const cache = createStorely(new Map());
 * await cache.set('user:1', { name: 'John' });
 *
 * // Create with namespace for multi-tenant scenarios
 * const tenantCache = createStorely(new Map(), {
 *   namespace: 'tenant-123',
 *   keySeparator: ':'
 * });
 * ```
 */
export function createStorely(store: StorelyMapType, options?: StorelyMemoryAdapterOptions) {
	const { namespace, ...adapterOptions } = options ?? {};
	const memoryAdapter = new StorelyMemoryAdapter(store, adapterOptions);
