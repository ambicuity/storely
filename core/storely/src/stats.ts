...
/**
 * Structure of a telemetry event emitted by Storely.
 */
export type StorelyTelemetryEvent = {
	/** The event type (e.g. "hit", "miss", "set", "delete", "error"). */
	event: string;
	/** The cache key involved, if applicable. */
	key?: string;
	/** The namespace of the Storely instance. */
	namespace?: string;
	/** Unix timestamp in milliseconds when the event occurred. */
	timestamp: number;
};

export type StorelyStatsOptions = {
	/**
	...
	maxEntries?: number;
	/**
	 * The event emitter (e.g. a Storely instance) to subscribe to for telemetry events.
	 * If provided, StorelyStats will automatically subscribe on construction.
	 */
	emitter?: IEventEmitter;
};

export class StorelyStats {
	private _hits = 0;
	private _misses = 0;
	private _sets = 0;
	private _deletes = 0;
	...
	private readonly _listeners = new Map<string, (event: StorelyTelemetryEvent) => void>();

	constructor(options?: StorelyStatsOptions) {
	 ...
	 */
	public buildKeyEventName(event: StorelyTelemetryEvent): string {
	...
	/**
	 * Subscribe to telemetry events from an emitter (e.g. a Storely instance).
