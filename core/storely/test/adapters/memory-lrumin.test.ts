...
describe("StorelyMemoryAdapter with lru.min - LRU Eviction Behavior", () => {
	...
	test("should support onEviction callback", async () => {
		...
		await storely.set(key1, value1);
		await storely.set(key2, value2);
		await storely.set(key3, value3); // This should evict key1
...
describe("StorelyMemoryAdapter with lru.min - createStorely() Integration", () => {
	...
	test("should handle TTL with createStorely", async () => {
		const lru = createLRU<string, unknown>({ max: 100 });
		const storely = createStorely(lru);
		...
		await storely.set(key, value, 10);
		await sleep(20);
