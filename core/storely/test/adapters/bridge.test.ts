...
import { StorelyBridgeAdapter, type StorelyBridgeStore } from "../../src/adapters/bridge.js";
import { StorelyEvents } from "../../src/types/storely.js";
...
describe("StorelyBridgeAdapter - Constructor and Options", () => {
	test("should accept a store, allow setting a new store, and detect capabilities", () => {
		const store = createMinimalStore();
		const bridge = new StorelyBridgeAdapter(store);
		...
		const fullBridge = new StorelyBridgeAdapter(createFullStore());
	...
	test("namespace and keySeparator options, defaults, and setters", () => {
		...
		const bridge1 = new StorelyBridgeAdapter(store, { namespace: ns, keySeparator: "::" });
		expect(bridge1.namespace).toBe(ns);
		expect(bridge1.keySeparator).toBe("::");

		const bridge2 = new StorelyBridgeAdapter(store);
...
describe("StorelyBridgeAdapter - Key Prefix", () => {
	test("getKeyPrefix with and without namespace and custom separator", () => {
		...
		const bridge2 = new StorelyBridgeAdapter(store, { keySeparator: "::" });
	...
	test("getKeyPrefixData with and without namespace", () => {
		...
		const ns = faker.string.alphanumeric(8);
		const key = faker.string.uuid();
		const bridge = new StorelyBridgeAdapter(store, { namespace: ns });
		...
		const bridge3 = new StorelyBridgeAdapter(store, { namespace: "ns1" });
		expect(bridge3.getKeyPrefixData("other:foo")).toEqual({ key: "other:foo" });
	});
});

describe("StorelyBridgeAdapter - Core Operations (minimal store)", () => {
	test("should set, get, delete, and clear values with and without namespace", async () => {
		...
		// With namespace
		const ns = faker.string.alphanumeric(8);
		const nsBridge = new StorelyBridgeAdapter(store, { namespace: ns });
	...
	test("should pass ttl to store set", async () => {
		...
		const store: StorelyBridgeStore = {
			get: vi.fn(),
			set: setSpy,
			delete: vi.fn(),
			clear: vi.fn(),
...
describe("StorelyBridgeAdapter - TTL / Expiration", () => {
...
describe("StorelyBridgeAdapter - Fallback Methods (minimal store)", () => {
	...
	test("deleteMany should emit error and return false on failure", async () => {
		...
		bridge.on(StorelyEvents.ERROR, (error: unknown) => {
			errors.push(error);
		});
...
describe("StorelyBridgeAdapter - Native Delegation (full store)", () => {
	test("has and hasMany should delegate with correct key prefixes", async () => {
		...
		// With namespace
		const nsBridge = new StorelyBridgeAdapter(createFullStore(), { namespace: "ns" });
...
describe("StorelyBridgeAdapter - Iterator", () => {
	test("should iterate, filter by namespace, and handle expired/non-array entries", async () => {
		...
		// Namespace filtering
		const nsBridge = new StorelyBridgeAdapter(createFullStore(), { namespace: "ns1" });
		...
		const expBridge = new StorelyBridgeAdapter(expStore);
...
describe("StorelyBridgeAdapter - Clear with Namespace", () => {
	test("should clear namespaced keys or entire store depending on config", async () => {
		...
		const bridge1 = new StorelyBridgeAdapter(store1, { namespace: "ns1" });
		...
		const bridge2 = new StorelyBridgeAdapter(store2);
		...
		const bridge3 = new StorelyBridgeAdapter(store3, { namespace: "ns1" });
	...
	test("should handle non-string and non-array iterator entries during clear", async () => {
		...
		const store1: StorelyBridgeStore = {
			get: vi.fn(),
			set: vi.fn(),
		...
		// Non-array entries
		const store2: StorelyBridgeStore = {
			get: vi.fn(),
			set: vi.fn(),
			delete: vi.fn().mockResolvedValue(true),
			clear: vi.fn(),
		...
		const bridge2 = new StorelyBridgeAdapter(store2, { namespace: "ns1" });
		await bridge2.clear();
		expect(store2.delete).toHaveBeenCalledTimes(2);
	});
});

describe("StorelyBridgeAdapter - Namespace Isolation", () => {
	test("should isolate keys between namespaces", async () => {
		const store = createMinimalStore();
		const bridge1 = new StorelyBridgeAdapter(store, { namespace: "ns1" });
		const bridge2 = new StorelyBridgeAdapter(store, { namespace: "ns2" });
...
describe("StorelyBridgeAdapter - Error Event Forwarding", () => {
	test("should forward error events from store and handle stores without on method", () => {
		...
		const storeErrorListeners = listeners.get(StorelyEvents.ERROR) ?? [];
		...
		expect(() => new StorelyBridgeAdapter(createMinimalStore())).not.toThrow();
	});
});

describe("StorelyBridgeAdapter - v5 Adapter Compatibility", () => {
	test("should accept store with opts and pass namespace to iterator", async () => {
		...
		// Without namespace
		const bridge3 = new StorelyBridgeAdapter(store2);
		iteratorSpy.mockClear();
