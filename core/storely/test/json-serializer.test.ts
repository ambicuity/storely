...
import { jsonSerializer, StorelyJsonSerializer } from "../src/json-serializer.js";

afterEach(() => {
	vi.unstubAllGlobals();
});

describe("StorelyJsonSerializer", () => {
	it("should be instantiable and have a default instance", () => {
		expect(new StorelyJsonSerializer()).toBeInstanceOf(StorelyJsonSerializer);
		expect(jsonSerializer).toBeInstanceOf(StorelyJsonSerializer);
	});
