...
/**
 * MySQL storage adapter for Storely.
 * Provides a persistent key-value store using MySQL as the backend.
 */
export class StorelyMysql extends Hookified implements StorelyStorageAdapter {
	/**
	...
	/**
	 * Creates a new StorelyMysql instance.
	 * @param options - Configuration options or connection URI string
	 */
	constructor(options?: StorelyMysqlOptions | string) {
	...
	public async getMany<Value>(keys: string[]) {
		...
		return strippedKeys.map(
			(key) => validMap.get(key) as StorelyStorageGetResult<Value | undefined>,
		);
	}
	 ...
	 */
	private generateMySqlOptions(options: StorelyMysqlOptions): ConnectionOptions {
...
/**
 * Helper function to create a Storely instance with StorelyMysql as the storage adapter.
 * @param options - Optional {@link StorelyMysqlOptions} configuration object or connection URI string.
 * @returns A new Storely instance backed by MySQL.
 */
export const createStorely = (options?: StorelyMysqlOptions | string) =>
