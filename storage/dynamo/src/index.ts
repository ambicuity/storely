...
export class StorelyDynamo extends Hookified implements StorelyStorageAdapter {
	...
	constructor(options: StorelyDynamoOptions | string) {
		...
		this._client = DynamoDBDocument.from(new DynamoDB(clientConfig));

		this._tableReady = this.ensureTable(this._tableName).catch((error: unknown) => {
	...
	/**
	 * Sets the DynamoDB endpoint URL.
	 */
		...
		try {
			...
			for (let i = 0; i < putRequests.length; i += chunkSize) {
				...
				const batchWriteInput: BatchWriteCommandInput = {
					RequestItems: {
						[this._tableName]: chunk,
					},
				};

				const response = await this._client.batchWrite(batchWriteInput);
				const unprocessed = response.UnprocessedItems?.[this._tableName];
				if (unprocessed) {
	...
	public async get<Value>(key: string): Promise<StorelyStorageGetResult<Value>> {
		try {
			...
			return Item.value as StorelyStorageGetResult<Value>;
			/* v8 ignore start -- @preserve */
		} catch (error) {
			this.emit("error", error);
			return undefined as StorelyStorageGetResult<Value>;
	...
	public async getMany<Value>(
		...
		try {
			...
			this.emit("error", error);
			return keys.map(() => undefined as StorelyStorageGetResult<Value>);
		}
		/* v8 ignore stop -- @preserve */
	}
...
/**
 * Creates a Storely instance with the DynamoDB adapter.
 * @param options - Options for the adapter including DynamoDB client configuration and table settings.
 * @returns A Storely instance with the DynamoDB adapter
 */
export function createStorely(options?: StorelyDynamoOptions | string): Storely {
	const adapter = new StorelyDynamo(options ?? {});
