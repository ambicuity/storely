...
import type { StorelyEntry, StorelyStorageAdapter } from "@ambicuity/ambicore";
import {
	createClient,
	createCluster,
	createSentinel,
...
// KeyDB is wire-compatible with Redis. The @redis/client URL parser only
// accepts redis://[s] schemes, so translate the keydb-flavored aliases.
function normalizeKeyDBUrl(url: string): string {
	if (url.startsWith("keydb://")) return `redis://${url.slice("keydb://".length)}`;
	if (url.startsWith("keydbs://")) return `rediss://${url.slice("keydbs://".length)}`;
	return url;
}
...
export default class StorelyKeyDB<T> extends Hookified implements StorelyStorageAdapter {
	 ...
	 * @param {string | RedisClientOptions | RedisClientType} [connect] How to connect to the KeyDB server. If string pass in the url, if object pass in the options, if RedisClient pass in the client.
		...
		if (connect) {
			if (typeof connect === "string") {
				...
				if ((connect as any).sentinelRootNodes !== undefined) {
					...
					const opts = connect as RedisClientOptions;
					const normalized =
						typeof opts.url === "string" ? { ...opts, url: normalizeKeyDBUrl(opts.url) } : opts;
					this._client = createClient(normalized) as RedisClientType;
				} else {
	...
	/**
	 * Get if Unlink is used instead of Del for clearing keys. This is more performant but may not be supported by all KeyDB versions.
	 * @default true
	 */
	...
	/**
	 * Set if Unlink is used instead of Del for clearing keys. This is more performant but may not be supported by all KeyDB versions.
	 */
	...
	/**
	 * Get the KeyDB URL used to connect to the server. This is used to get a connected client.
	 */
	...
	/**
	 * Disconnect from the KeyDB server.
...
export { Storely } from "@ambicuity/ambicore";
export {
	createClient,
	createCluster,
	createSentinel,
