...
describe("set", () => {
	...
	test("should throw error on bad uri", async () => {
		...
		vi.spyOn(storelyRedis.client, "set").mockImplementation(() => {
	...
	test("should set a value with ttl", async () => {
		...
		await storelyRedis.set(data.key, data.value, data.ttl);
		...
		const expiredResult = await storelyRedis.get(data.key);
	...
	test("show throw on redis client set and get error", async () => {
		...
		expect(didError).toBe(true);
		vi.spyOn(storelyRedis.client, "set").mockRestore();
	});
	...
	test("show throw on redis client setMany and get error", async () => {
		...
		let didError = false;
		try {
			await storelyRedis.setMany([data, data, data]);
		} catch {
			didError = true;
		}
	...
	test("setMany should return false entries on error when throwOnErrors is false", async () => {
		const storelyRedis = new StorelyRedis(redisUri);
		storelyRedis.on("error", () => {}); // Silence expected errors
		...
		const result = await storelyRedis.setMany([data, data, data]);
	...
	test("should be able to set a ttl", async () => {
		...
		await storelyRedis.set(key, faker.lorem.word(), 10);
		await delay(15);
