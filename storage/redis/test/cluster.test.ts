...
// (nodes on 7001/7002/7003). Gate behind STORELY_REDIS_CLUSTER=1 so the
// default `pnpm test:ci` doesn't hang waiting on missing infra.
// To run: STORELY_REDIS_CLUSTER=1 pnpm test:ci.
const describeCluster = process.env.STORELY_REDIS_CLUSTER === "1" ? describe : describe.skip;
...
describeCluster("StorelyRedis Cluster", () => {
	beforeEach(async () => {
