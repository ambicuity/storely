...
* BREAKING: YOUR PREVIOUS KEYS WILL NOT BE VALID. This is because of the fixe of the namespace support and how it is handled. Now, when using `storely` with `storely/redis` as the storage adapter you can do the following:
...
This will make it so the storage adapter `storely/redis` will handle the namespace and not the `storely` instance. If you leave it on it will just look duplicated like `my-namespace:my-namespace:key`.
