 ...
 */
export class StorelyMongo extends Hookified implements StorelyStorageAdapter {
	/**
	 ...
	 */
	public connect: Promise<StorelyMongoConnect>;
	...
	/**
	 * Creates a new StorelyMongo instance.
	 ...
	 */
	constructor(url?: StorelyMongoOptions, options?: Options) {
	...
	public async get<Value>(key: string): Promise<StorelyStorageGetResult<Value>> {
		...
		if (this._useGridFS) {
			...
			return new Promise((resolve) => {
				...
				stream.on("end", () => {
					...
					resolve(data as StorelyStorageGetResult<Value>);
				});
		...
		return document.value as StorelyStorageGetResult<Value>;
	}
	...
	public async getMany<Value>(keys: string[]) {
		...
		const now = new Date();
		const validMap = new Map<string, StorelyStorageGetResult<Value>>();
		...
		for await (const doc of cursor) {
			if (doc.expiresAt && new Date(doc.expiresAt as Date) <= now) {
				...
				validMap.set(doc.key as string, doc.value as StorelyStorageGetResult<Value>);
			}
		}
		...
		return strippedKeys.map((key) => validMap.get(key) as StorelyStorageGetResult<Value>);
	}
	...
	/**
	 * Strips the namespace prefix from a key that was added by the Storely core.
	...
	/**
	 * Extracts MongoDB driver options from the provided options, filtering out Storely-specific properties.
	 */
	private extractMongoOptions(options: Options): MongoClientOptions {
		const storelyKeys = new Set([
			"url",
			"collection",
			"namespace",
			"serialize",
...
/**
 * Helper function to create a Storely instance with StorelyMongo as the storage adapter.
 * @param options - Optional {@link StorelyMongoOptions} configuration object or connection URI string.
 * @returns A new Storely instance backed by MongoDB.
 */
export const createStorely = (options?: StorelyMongoOptions) => {
	const store = new StorelyMongo(options);
	const namespace = typeof options === "object" ? options?.namespace : undefined;
	return new Storely({ store, namespace });
};

export default StorelyMongo;
export type { StorelyMongoOptions } from "./types.js";
