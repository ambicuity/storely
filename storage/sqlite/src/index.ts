 ...
 * ```
 */
export class StorelySqlite extends Hookified implements StorelyStorageAdapter {
	...
	/**
	 * Creates a new StorelySqlite instance.
	 *
	 ...
	 *
	 * @param storelyOptions - A SQLite connection URI string (e.g. `'sqlite://path/to/db.sqlite'`)
	 *   or a {@link StorelySqliteOptions} configuration object. Defaults to an in-memory database.
	 ...
	 */
	constructor(storelyOptions?: StorelySqliteOptions | string) {
			...
			.then(async (database) => {
				...
				if (tableInfo.length === 0) {
					...
					if (!columnNames.includes("namespace")) {
						...
						// Old keys are stored as "namespace:actualKey" (e.g. "storely:foo").
	...
	async getMany<Value>(keys: string[]) {
		...
		return strippedKeys.map(
			(key) => (validMap.get(key) ?? undefined) as StorelyStorageGetResult<Value | undefined>,
		);
	}
	...
	/**
	 * Applies configuration options from a partial {@link StorelySqliteOptions} object.
	 ...
	 */
	private setOptions(options: StorelySqliteOptions): void {
...
/**
 * Helper function to create a Storely instance with StorelySqlite as the storage adapter.
 * @param storelyOptions - Optional {@link StorelySqliteOptions} configuration object or URI string.
 * @returns A new Storely instance backed by SQLite.
 */
export const createStorely = (storelyOptions?: StorelySqliteOptions | string) =>
	new Storely({ store: new StorelySqlite(storelyOptions) });

export default StorelySqlite;
export type {
	Sqlite3DatabaseLike,
	Sqlite3ModuleLike,
