...
import StorelyValkey from "../src/index.js";
...
it("cluster: setMany should work without CROSSSLOT errors", { retry: 3 }, async (t) => {
	...
	const results = await storely.getMany([key1, key2, key3, key4]);
...
it("cluster: deleteMany should work without CROSSSLOT errors", { retry: 3 }, async (t) => {
	...
	const result = await storely.deleteMany([key1, key2, key3]);
	...
	t.expect(await storely.get(key3)).toBe(undefined);
