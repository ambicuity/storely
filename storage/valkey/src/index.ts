...
/**
 * Valkey storage adapter for Storely. Supports both standalone and cluster modes
 * using iovalkey as the underlying client. Implements the {@link StorelyStorageAdapter}
 * interface with support for namespacing, TTL, batch operations, and async iteration.
 */
class StorelyValkey extends Hookified implements StorelyStorageAdapter {
	/**
	 ...
	 */
	constructor(uri: StorelyValkeyOptions | StorelyUriOptions, options?: StorelyValkeyOptions) {
	 ...
	 * @returns {Promise<StorelyStorageGetResult<Value>>} The stored data if found, or `undefined` if the key does not exist.
	 */
	public async get<Value>(key: string): Promise<StorelyStorageGetResult<Value>> {
	 ...
	 * @returns {Promise<Array<StorelyStorageGetResult<Value | undefined>>>} An array of stored data in the same order as the input keys.
	...
	public async getMany<Value>(
		...
		if (this.isCluster()) {
			...
			const resultMap = new Map<string, StorelyStorageGetResult<Value | undefined>>();
			...
			return resolvedKeys.map(
				(k) => resultMap.get(k) as StorelyStorageGetResult<Value | undefined>,
			);
		}
...
/**
 * Creates a new {@link Storely} instance pre-configured with the Valkey storage adapter.
 * This is a convenience factory function that handles adapter instantiation and wiring.
 * @param {StorelyValkeyOptions | StorelyUriOptions} [connect="redis://localhost:6379"] - Connection configuration.
 *   Can be a Redis URI string (e.g. `"redis://localhost:6379"`) or an options object with
 *   connection details and adapter settings.
 * @param {StorelyValkeyOptions} [options] - Additional adapter options such as `useSets` and `namespace`.
 * @returns {Storely} A fully configured Storely instance backed by the Valkey adapter.
 * @example
 * ```typescript
 * const storely = createStorely("redis://localhost:6379");
 * await storely.set("greeting", "hello");
 * console.log(await storely.get("greeting")); // "hello"
 * ```
 */
export function createStorely(
	connect?: StorelyValkeyOptions | StorelyUriOptions,
	options?: StorelyValkeyOptions,
): Storely {
	connect ??= "redis://localhost:6379";
	const adapter = new StorelyValkey(connect, options);
	const storely = new Storely(adapter);
	return storely;
}

export default StorelyValkey;
export type { StorelyValkeyOptions } from "./types.js";
