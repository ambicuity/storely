...
### Build
- `pnpm build` - Build all packages (storely first, then all others)
- `pnpm build:storely` - Build only the storely package
...
**Core Storely Class** (`core/storely/src/index.ts`):
- Extends EventManager for event emission
- Uses HooksManager for pre/post operation hooks
- Includes StorelyStats for usage statistics
...
**Serialization**:
- Default uses built-in `StorelyJsonSerializer` with JSON.stringify/parse
- Compression adapters can be plugged in
- Data format: `{ value: T, expires?: number }`

### Build Dependencies
1. `storely` core must be built first (used by adapters and serialization packages)
...
- Core package (`storely`) has no external dependencies (serializer is built-in)
- Storage adapters depend on `storely` as peer dependency
- Test suite depends on `storely` and various testing utilities
...
- `storely - docs: Update API documentation`
- `mono - chore: Upgrade dependencies`

**Package names:**
- Use the package name (e.g., `sqlite`, `redis`, `postgres`, `storely`, `serialize`)
