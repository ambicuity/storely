...
import { msgpackrSerializer, StorelyMsgpackrSerializer } from "../src/index.js";

// Standard serialization compliance tests
serializationTestSuite(it, msgpackrSerializer);

describe("StorelyMsgpackrSerializer", () => {
	it("should be instantiable", () => {
		const serializer = new StorelyMsgpackrSerializer();
		expect(serializer).toBeInstanceOf(StorelyMsgpackrSerializer);
	});

	it("msgpackrSerializer is a default instance", () => {
		expect(msgpackrSerializer).toBeInstanceOf(StorelyMsgpackrSerializer);
	});
});
